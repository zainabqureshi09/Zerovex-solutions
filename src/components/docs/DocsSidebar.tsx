...
export function DocsSidebar() {
  ...
  return (
    <aside className="w-[280px] h-[calc(100vh-57px)] sticky top-[57px] overflow-y-auto border-r border-[var(--color-docs-sidebar-border)] dark:border-[var(--color-docs-sidebar-border-dark)] bg-[var(--color-docs-sidebar)] dark:bg-[var(--color-docs-sidebar-dark)] p-4">
          ...
          <div key={lang.name} className="mb-2">
            <button
              ...
              className={cn(
                ...
                "hover:bg-[var(--color-docs-sidebar-hover)] dark:hover:bg-[var(--color-docs-sidebar-hover-dark)]",
                expandedLanguages.includes(lang.name) && "bg-[var(--color-docs-sidebar-active)] dark:bg-[var(--color-docs-sidebar-active-dark)]"
              )}
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-[var(--color-docs-foreground)] dark:text-[var(--color-docs-foreground-dark)]">{lang.name}</span>
              </div>
              {expandedLanguages.includes(lang.name) ? (
                <ChevronDown className="w-4 h-4 text-[var(--color-docs-muted-foreground)] dark:text-[var(--color-docs-muted-foreground-dark)]" />
              ) : (
                <ChevronRight className="w-4 h-4 text-[var(--color-docs-muted-foreground)] dark:text-[var(--color-docs-muted-foreground-dark)]" />
              )}
            </button>

            {expandedLanguages.includes(lang.name) && (
              <div className="ml-4 mt-1 space-y-1 border-l border-[var(--color-docs-border)] dark:border-[var(--color-docs-border-dark)] pl-3">
                {lang.sections.map((section) => (
                  <div key={section.title}>
                    <button
                      onClick={() => toggleSection(section.title)}
                      className="w-full flex items-center justify-between px-2 py-1.5 text-xs font-medium text-[var(--color-docs-muted-foreground)] dark:text-[var(--color-docs-muted-foreground-dark)] uppercase tracking-wide"
                    >
                      {section.title}
                        ...
                        {section.items.map((item) => (
                          <Link
                            ...
                            className={cn(
                              ...
                              "text-[var(--color-docs-muted-foreground)] dark:text-[var(--color-docs-muted-foreground-dark)]",
                              "hover:bg-[var(--color-docs-sidebar-hover)] dark:hover:bg-[var(--color-docs-sidebar-hover-dark)] hover:text-[var(--color-docs-foreground)] dark:hover:text-[var(--color-docs-foreground-dark)]",
